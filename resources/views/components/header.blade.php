<div class="container vh-100 w-75 my-5">
    <div class="row border h-50 rounded">
        <div class="col-12 col-md-6 d-flex flex-column justify-content-around align-items-center">
            <h1>Lorem cdaasad</h1>

            <form class="d-flex" role="search">
                <input class="form-control me-2" type="search" placeholder="Search" aria-label="Search">
                <button class="btn btn-outline-success" type="submit">Search</button>
            </form>
        </div>

        <div class="col-12 col-md-6 d-flex justify-content-center">
            <img src="https://picsum.photos/200/200" class="img-fluid rounded-circle p-4" alt="">
        </div>
    </div>

    <div class="container headerCustom">
        <div class="row my-4">
            <div class="col-12 col-md-6 d-flex justify-content-around">
            

                <div class="border d-flex flex-column align-items-center" data-aos="fade-right" data-aos-offset="300" data-aos-easing="ease-in-sine">
                    <h3>Motori</h3>
                    <i class="fa-solid fa-car"></i>
                </div>

                <div class="border d-flex flex-column align-items-center" data-aos="fade-right" data-aos-offset="300" data-aos-easing="ease-in-sine">
                    <h3>Abbigliamento</h3>
                    <i class="fa-sharp fa-solid fa-shirt"></i>

                </div>

            </div>

            <div class="col-12 col-md-6 d-flex justify-content-around">

                <div class="border d-flex flex-column align-items-center" data-aos="fade-left" data-aos-offset="300" data-aos-easing="ease-in-sine">
                    <h3>Arredamento</h3>
                    <i class="fa-solid fa-couch" style="color: #000000;"></i>

                </div>

                <div class="border d-flex flex-column align-items-center" data-aos="fade-left" data-aos-offset="300" data-aos-easing="ease-in-sine">
                    <h3>Tecnologia</h3>
                    <i class="fa-solid fa-laptop" style="color: #000000;"></i>

                </div>

            </div>
        </div>
        
        <div class="row my-4">
            <div class="col-12 col-md-6 d-flex justify-content-around">

                <div class="border d-flex flex-column align-items-center" data-aos="fade-right" data-aos-offset="300" data-aos-easing="ease-in-sine">
                    <h3>Musica</h3>
                    <i class="fa-solid fa-music"></i>

                </div>

                <div class="border d-flex flex-column align-items-center" data-aos="fade-right" data-aos-offset="300" data-aos-easing="ease-in-sine">
                    <h3>Attrezzatura</h3>
                    <i class="fa-solid fa-toolbox"></i>

                </div>

            </div>

            <div class="col-12 col-md-6 d-flex justify-content-around">

                <div class="border d-flex flex-column align-items-center" data-aos="fade-left" data-aos-offset="300" data-aos-easing="ease-in-sine">
                    <h3>Sport</h3>
                    <i class="fa-solid fa-futbol" style="color: #000000;"></i>

                </div>

                <div class="border d-flex flex-column align-items-center" data-aos="fade-left" data-aos-offset="300" data-aos-easing="ease-in-sine">
                    <h3>Film</h3>
                    <i class="fa-solid fa-clapperboard" style="color: #000000;"></i>

                </div>

            </div>
        </div>
    </div>
</div>